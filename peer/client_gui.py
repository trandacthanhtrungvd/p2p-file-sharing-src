...
class PublishFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.lname_entry.grid(row=1, padx=8, pady=(0, 16), sticky="w")
        ...
        self.fname_entry.grid(row=3, padx=8, pady=(0, 16), sticky="w")
...
class FetchFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.fname_entry = customtkinter.CTkEntry(self, width=340, height=40, placeholder_text="Input File Name...")
        self.fname_entry.grid(row=1, padx=8, pady=(0, 16), sticky="we")
...
class App(Tk):
    def __init__(self):
        ...
        # UI elements
        self.start_button = customtkinter.CTkButton(self, text="Start Sharing", width=103, height=32, command=self.start_button_callback)
        self.start_button.grid(row=0, column=0, padx=(32, 0), pady=32, sticky="w")

        self.shutdown_button = customtkinter.CTkButton(self, text="Stop Sharing", width=103, height=32, state='disabled', command=self.shutdown_button_callback)
        self.shutdown_button.grid(row=0, column=1, padx=(32, 0), pady=32, sticky="w")

        # Frames
        self.server_frame = CentralizedServerFrame(self)
        self.server_frame.grid(row=0, column=3, columnspan=2, padx=(12, 32), pady=32, sticky="news")

        self.publish_frame = PublishFrame(self)
        self.publish_frame.grid(row=1, column=0, columnspan=2, padx=(32, 12), pady=(0, 32), sticky="news")

        self.fetch_frame = FetchFrame(self)
        self.fetch_frame.grid(row=1, column=3, columnspan=2, padx=(12, 32), pady=(0, 32), sticky="news")
